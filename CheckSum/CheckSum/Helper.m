...
+ (int)OTACheckSumWithFileStr:(NSString *)filepath
{
    ...
    @try {
        ...
        NSData *data1 = [[NSData alloc] initWithBytes:bytes length:Size];
        
        for (int i = 0; i < len; i++) {
//                temp += *((UInt32*)bytes + i*4);
//            temp += [self unsignedDataTointWithData:data1 Location:i * 4 Offset:4];
            temp += [self signedDataTointWithData:data1 Location:i * 4 Offset:4];
        }
        
        NSLog(@"%ld",temp);
        NSLog(@"%ld",TempMask);
        CheckSum = (int)(temp & TempMask);
        NSLog(@"%d",0x7fffffff&CheckSum);
